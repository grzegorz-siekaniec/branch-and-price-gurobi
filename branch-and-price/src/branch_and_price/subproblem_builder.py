...
class SubproblemBuilder:
    ...
    def _build_capacity_constraint(self,
                                    model: grb.Model,
                                    machine_id: int,
                                    task_machine_to_variable: Dict[Tuple[int, int]]):
        lhs = grb.quicksum([
            self._gap_instance.weight(task_id, machine_id) * task_machine_to_variable[(task_id, machine_id)]
    ...
    def _build_columns(self, model, machine_id, task_duals, branching_rules) -> Dict[Tuple[int, int]]:
        task_to_variable: Dict[Tuple[int, int]] = dict()
        ...
        for task_id in range(self._gap_instance.num_tasks):
            ...
            task_to_variable[assignment] = var
